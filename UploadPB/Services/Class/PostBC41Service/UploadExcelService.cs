...
namespace UploadPB.Services.Class.PostBC41Service
{
    public class UploadExcelService : IUploadExcel41
    {
        private readonly SupportDbContext context;
        private readonly DbSet<Beacukai41Temporary> dbSet;
        ...
        public UploadExcelService(IServiceProvider provider, SupportDbContext context)
        {
            ...
            this.dbSet = context.Set<Beacukai41Temporary>();
        ...
        public async Task<int> Upload(ExcelWorksheets sheet)
        {
            ...
            using (var transaction = this.context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var itemtoDelete = context.Set<Beacukai41Temporary>();
                    context.beacukai41Temporaries.RemoveRange(itemtoDelete);
                    context.SaveChanges();
                    transaction.Commit();
        ...
        public async Task<int> InsertToTemporary(List<TemporaryViewModel> data)
        {
            ...
            using (var transaction = this.context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        Beacukai41Temporary beacukaiTemporaryModel = new Beacukai41Temporary
                        {
                            ID = index++,
                            BCNo = a.BCNo,
                            Barang = a.Barang,
                ...
                for (rowIndex = 2; rowIndex <= totalRow; rowIndex++)
                {
                    if (sheet.Cells[rowIndex, 94].Value != null)
                    {
                        listData.Add(new HeaderDokumenTempModel(
                            ...
                            converterChecker.GenerateValueDecimal(sheet.Cells[rowIndex, 74]),//HARGA_PENYERAHAN
                            null,//CIF_Rupiah
