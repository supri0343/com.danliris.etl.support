...
namespace UploadPB.Services.Class.Post41
{
    public class PostBeacukaiService : IPostBeacukai41
    {
                ...
                try
                {
                    ...
                    foreach (var item in data)
                    {
                        ...
                        if (!existAjuBC.Contains(item.NoAju))
                        {
                            ...
                            var ver = context.beacukai41Temporaries.Select(x => x).Where(x => x.NoAju == item.NoAju).ToList();
