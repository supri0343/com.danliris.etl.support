...
{
    public class UploadExcelService : IUploadExcel23
    {
        private readonly SupportDbContext context;
        private readonly DbSet<Beacukai23Temporary> dbSet;
        ...
        public UploadExcelService(IServiceProvider provider, SupportDbContext context)
        {
            ...
            this.dbSet = context.Set<Beacukai23Temporary>();
        ...
        public async Task<int> InsertToTemporary(List<TemporaryViewModel> data)
        {
            ...
            using (var transaction = this.context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        Beacukai23Temporary beacukaiTemporaryModel = new Beacukai23Temporary
                        {
                            ID = index++,
                            BCNo = a.BCNo,
                            Barang = a.Barang,
        ...
        public List<EntitasTemp> UploadEntitas(ExcelWorksheets excel, int data)
        {
            ...
            try
            {
                ...
                for (rowIndex = 2; rowIndex <= totalRow; rowIndex++)
                {
                    ...
                    if (sheet.Cells[rowIndex, 1].Value != null && converterChecker.GenerateValueInt(sheet.Cells[rowIndex, 3]) == 5) 
                    {
