...
{
    public class UploadExcelService : IUploadExcel20
    {
        private readonly AGDbContext context;
        private readonly SupportDbContext SupportContext;
        private readonly DbSet<Beacukai20Temporary> dbSet;
        private readonly DbSet<BeacukaiDocumentsModel> beacukaiDocuments;

        ConverterChecker converterChecker = new ConverterChecker();
        
        public UploadExcelService(IServiceProvider provider, AGDbContext context, SupportDbContext supportDbContext)
        {
            this.context = context;
            this.dbSet = context.Set<Beacukai20Temporary>();
            this.beacukaiDocuments = supportDbContext.Set<BeacukaiDocumentsModel>();
        }
        ...
        public async Task<int> InsertToTemporary(List<TemporaryViewModel> data)
        {
            ...
            using (var transaction = this.context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        Beacukai20Temporary beacukaiTemporaryModel = new Beacukai20Temporary
                        {
                            ID = index++,
                            BCNo = a.BCNo,
                            Barang = a.Barang,
        ...
        public List<EntitasTemp> UploadEntitas(ExcelWorksheets excel, int data)
        {
            ...
            try
            {
                ...
                for (rowIndex = 2; rowIndex <= totalRow; rowIndex++)
                {
                    ...
                    if (sheet.Cells[rowIndex, 1].Value != null && converterChecker.GenerateValueInt(sheet.Cells[rowIndex, 3]) == 10)
                    {
