...
{
    public class UploadExcelService : IUploadExcel25
    {
        private readonly SupportDbContext context;
        private readonly DbSet<Beacukai25Temporary> dbSet;
        ...
        public UploadExcelService(IServiceProvider provider, SupportDbContext context)
        {
            ...
            this.dbSet = context.Set<Beacukai25Temporary>();
        ...
        public async Task<int> InsertToTemporary(List<TemporaryViewModel> data)
        {
            ...
            using (var transaction = this.context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        Beacukai25Temporary beacukaiTemporaryModel = new Beacukai25Temporary
                        {
                            ID = index++,
                            BCNo = a.BCNo,
                            Barang = a.Barang,
                ...
                for (rowIndex = 2; rowIndex <= totalRow; rowIndex++)
                {
                    if (sheet.Cells[rowIndex, 94].Value != null)
                    {
                        listData.Add(new HeaderDokumenTempModel(
                            ...
                            converterChecker.GenerateValueDecimal(sheet.Cells[rowIndex, 26]),//CIF
                            null,//CIF_Rupiah
                ...
                for (rowIndex = 2; rowIndex <= totalRow; rowIndex++)
                {
                    if (sheet.Cells[rowIndex, 2].Value != null)
                    {
                        listData.Add(new BarangTemp(
                              ...
                              converterChecker.GenerateValueDecimal(sheet.Cells[rowIndex, 34]),"",0
                            ));
                    }
                }
                return listData;
